...
/// using  vec2i_t   =   vec2i;
    ...
    ///-----------------------:
    struct  Config
    {       Config(const map_t& mp, wchar_t st) : pmap(&mp), STATUS(size_t(st))
            {
                if(pmap != nullptr)
    ...
    {       Cell() = default;
        ...
        void   cls(const Config&   cfg)     { this->status = cfg.STATUS; }
    ...
    {       EmDisplay(const Config& cfg) :
                matrix_t(cfg.sizedisplay.y,
    ...
    {       Field(const Config& _cfg) :

                    matrix_t(_cfg.sizefield.y,
                    massiv_t(_cfg.sizefield.x, Cell(_cfg.STATUS))),
                    cfg     (_cfg),
                    mat     (*this)
            {
                init        (mat);
            ...
            Field(const Config& _cfg, const map_t* Map) :
                cfg(_cfg),
                mat(*this)
            {   load(Map);
                init(mat);
    ...
    ///------------------------------------------------------------------ Snake:
    struct  Snake
    {       Snake  (const Config& _cfg, Field& f, const vec2i_t& position)
                :   cfg  (_cfg),
                    field( f  ),
        ...
        {   const auto& pos = body.back().pcell->position;
            field[pos.y][pos.x].cls(field.cfg);
    ...
    void Snake::test()
    {
        ...
        EmDisplay display(config1);
        Field       field(config1);
        Apple       apple;
                    apple.set(field.getfree(), config1.APPLE);
        Snake snake(config1, field, {10,   10});
    ...
    ///------------------------------------------------------------------ Moved:
    struct  Moved
    {       Moved(const Config& _cfg) : cfg(_cfg)
            {
            }
    ...
    {       AIdefault(const Config& _cfg) : moved(_cfg)
            {   name = "AIdefault";
            }
        ...
        Cell* step(const Snake& snake)
        {
            ...
            {   auto n = size_t(rand()%dir.size());
                ...
                if(1 == dir.size())
                {
                    ...
                    isapple     = moved.cfg.is_apple(satatus);
                }
    ...
    ///------------------------------------------------------------------- Game:
    struct  Game
    {       Game(const Config& _cfg)
                :   cfg    (_cfg),
                    display(_cfg),
                    field(  _cfg , cfg.pmap),
                    snake(  _cfg , field, {10, 10})
            {
                setapple();
                pIAI = new AIdefault(cfg);
            }
           ~Game(){ delete pIAI; }
