...
{
    ////////////////////////////////////////////////////////////////////////////
    /// "Pixelate" fragment shader                                        ---> 0
    ////////////////////////////////////////////////////////////////////////////
    struct  Pixelate   : IEffect
    {       Pixelate() : IEffect("pixelate")
            {   load() ;
            }

        bool onLoad()
        {   if (!m_texture.loadFromFile(DIR+"resources/background.jpg"))
                return false;

            m_sprite .setTexture  (m_texture);

            const auto& sz = m_sprite.getGlobalBounds();
            m_sprite.setOrigin(sz.width / 2, sz.height / 2);

            if (!m_shader.loadFromFile(DIR+"resources/pixelate.frag", sf::Shader::Fragment))
                return false;
...
private:
    ...
    ////////////////////////////////////////////////////////////////////////////
    /// "Wave" vertex shader + "blur" fragment shader                     ---> 1
    ////////////////////////////////////////////////////////////////////////////
    struct  WaveBlur   : public IEffect
    {       WaveBlur() :        IEffect("wave + blur")
            {   load() ;
            }
...
"In hac habitasse platea dictumst. Etiam fringilla est id odio dapibus sit amet semper dui laoreet.\n");
            ...
            const auto& sz = m_text.getGlobalBounds();
            m_text.setOrigin(sz.width / 2, sz.height / 2);

            if (!m_shader.loadFromFile(DIR+"resources/wave.vert",
                                       DIR+"resources/blur.frag"))
                return false;
...
////////////////////////////////////////////////////////////////////////////////
/// "Storm" vertex shader + "blink" fragment shader                       ---> 2
////////////////////////////////////////////////////////////////////////////////
    struct  StormBlink   : public IEffect
    {       StormBlink() :        IEffect("storm + blink")
            {   load  () ;
            }
        ...
        {   // Create the points
            ...
            {   float x = static_cast<float>(std::rand() % 800 - 400);
                float y = static_cast<float>(std::rand() % 600 - 300);
            ...
            // Load the shader
            if (!m_shader.loadFromFile(DIR+"resources/storm.vert",
                                       DIR+"resources/blink.frag"))
                return false;
...
////////////////////////////////////////////////////////////////////////////////
/// "Edge" post-effect fragment shader                                    ---> 3
////////////////////////////////////////////////////////////////////////////////
    struct  Edge       : public IEffect
    {       Edge    () :        IEffect("edge post-effect")
            {   load() ;
            }
        ...
        {   // Create the off-screen surface
            ...
            // Load the textures
            if (!m_backgroundTexture.loadFromFile(DIR+"resources/sfml.png"))
                return false;

            m_backgroundTexture.setSmooth(true);
            if (!m_entityTexture.loadFromFile(DIR+"resources/devices.png"))
                return false;
            ...
            const auto& sz = m_backgroundSprite.getGlobalBounds();
            m_backgroundSprite.setOrigin(sz.width / 2, sz.height / 2);
            ...
            // Load the shader
            if (!m_shader.loadFromFile(DIR+"resources/edge.frag",
                                       sf::Shader::Fragment))
                return false;
            m_shader.setUniform("texture", sf::Shader::CurrentTexture);
...
////////////////////////////////////////////////////////////////////////////////
/// "Geometry" geometry shader example                                    ---> 4
////////////////////////////////////////////////////////////////////////////////
    struct  Geometry   : public IEffect
    {       Geometry() :        IEffect("geometry shader billboards"),
        ...
        {   // Check if geometry shaders are supported
            ...
            if (!m_logoTexture.loadFromFile(DIR+"resources/logo.png"))
                return false;

            // Load the shader
            if (!m_shader.loadFromFile(DIR+"resources/billboard.vert",
                                       DIR+"resources/billboard.geom",
                                       DIR+"resources/billboard.frag"))
                return false;
